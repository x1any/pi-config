...
`qmd init` creates a project-local `.qmd` index. Run it from the project or
knowledge-base directory that should own the index; skip it when intentionally
using an existing default or named QMD index.
...
- **Do not mutate indexes casually.** `qmd init`, `qmd collection add`,
  `qmd update`, and `qmd embed` change local state and can be expensive.
