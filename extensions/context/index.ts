...
function buildOverlay(
    ...
    // Legend — two columns
    const nonFreeCategories = breakdown.categories.filter(
        (c) => c.key !== "free",
    );
    const freeCat = breakdown.categories.find((c) => c.key === "free");
    ...
    const formatEntry = (cat: Category, w: number): string => {
        const pctStr = ((cat.tokens / breakdown.contextWindow) * 100).toFixed(
            1,
        );
        const label = `${cat.square} ${cat.color(theme, cat.label)}`;
        const value = theme.fg(
            "dim",
            `${formatTokens(cat.tokens)} (${pctStr}%)`,
        );
    ...
    if (freeCat && freeCat.tokens > 0) {
        const pctStr = (
            (freeCat.tokens / breakdown.contextWindow) *
            100
        ).toFixed(1);
    ...
    if (breakdown.percent !== null && breakdown.percent > 95) {
        suggestions.push(
            "🔴 Near context limit — compaction strongly recommended",
        );
    }

    // Find biggest tool consumer
    const toolCats = breakdown.categories.filter((c) =>
        c.key.startsWith("tool:"),
    );
